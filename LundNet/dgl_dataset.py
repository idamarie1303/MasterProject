...
from lundnets.dgl_utils import knn_graph
from torch.utils.data import Dataset
from lundnets.JetTree import JetTree, LundCoordinates
from lundnets.read_data import Jets
...
class DGLGraphDatasetLund(Dataset):
    ...
    def __init__(self, filepath_bkg, nev=-1):
        super(DGLGraphDatasetLund, self).__init__()
        print('Start loading dataset %s' % (filepath_bkg))
        ...
        for jet in reader_bkg:
            jet1, label, l_pt, nr_l = jet
            self.data += [self._build_tree(JetTree(jet1))]
        ...
        if dump_number_of_nodes:
            df = pd.DataFrame({'num_nodes': np.array(
                [g.number_of_nodes() for g in self.data]), 'label': np.array(self.label), 'pt': np.array(self.pt), 'nr': np.array(self.nr)})
            df.to_csv('num_nodes_lund_net_ktmin_%s_deltamin_%s.csv' % (JetTree.ktmin, JetTree.deltamin))
        self.label = torch.tensor(self.label, dtype=torch.float32)
        self.pt = torch.tensor(self.pt, dtype=torch.float32)
        self.nr = torch.tensor(self.nr, dtype=torch.float32)
...
class _LundTreeBatch:
    ...
    def __init__(self, data):
        ...
        #print([i[0] for i in transposed_data[1]])
        self.label = torch.tensor([i[0] for i in transposed_data[1]])
        
        self.pt = torch.tensor([i[1] for i in transposed_data[1]])
        self.nr = torch.tensor([i[2] for i in transposed_data[1]])
