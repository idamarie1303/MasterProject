...
from lundnets.EdgeConv import EdgeConvBlock
...
class LundNet(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_fusion:
            in_chn = sum(x[-1] for x in conv_params) #sum out channels of all layers
            out_chn = np.clip((in_chn // 128) * 128, 128, 1024) # becomes 384, supposed to regulate so out channels dont become less than 128, or more than 1024
    ...
    def forward(self, batch_graph, features):
        ...
        if self.use_fusion: #Concatenate + Aggreagation layer
