...
class Jets(Image):
    ...
    def __init__(self, infile, nmax, pseudojets=True, groomer=None):
        ...
        self.jet_def = fj.JetDefinition(fj.kt_algorithm, 1000.) #Radius 0.7, set to 1000. in original file??
        self.pseudojets = pseudojets
        self.groomer = groomer #None
        #print("kl")
    ...
    def process(self, event):
        ...
        if self.pseudojets or self.groomer:
            ...
            if (len(jets) > 0):
                ...
                return [jets[0], label, l_pt, l_nr]
            return [fj.PseudoJet(), label, l_pt, l_nr]
        else:
            for p in event[1:]:
                constits.append([p['px'], p['py'], p['pz'], p['E']])
            return [constits, label, l_pt, l_nr]
